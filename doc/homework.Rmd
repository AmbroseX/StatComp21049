...
```
alist <- ppoints(1000)
QRx <- quantile(dataR[1,],alist)
QCx <- quantile(dataC[1,],alist)
qqplot(QRx,QCx,main="qqplot for X",xlab = "X:Using R",ylab = "Y:Using Cpp")
abline(0,1,col="red")

QRy <- quantile(dataR[2,],alist)
QCy <- quantile(dataC[2,],alist)
qqplot(QRy,QCy,main="qqplot for Y",xlab = "Y:Using R",ylab = "Y:Using Cpp")
abline(0,1,col="red")
```
...
summary(ts)[,c(1,3,5,6)]
```
我的代码上面不知道为什么会导致`R Session Aborted`,如果可以你可以帮我运行一下
我自己写了一个测试函数运行时间的代码
结果显示在R和Rcpp生成的分布差不多，但是Rcpp明显计算快了非常多
------------------------------------------------------------------------
